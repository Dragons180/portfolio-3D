// scroll-anim.js — GSAP-driven scroll reveals (UMD globals)
(function(){
  'use strict';

  function whenReady(cb){
    const tick = () => {
      if (window.gsap && document.querySelector('[data-anim]')) cb(window.gsap);
        ...
        entries.forEach(entry => {
          if (entry.isIntersecting) {
            gsap.to(entry.target, {
              y: 0, opacity: 1,
              duration: 0.9, ease: 'expo.out', delay: 0.05
            });
